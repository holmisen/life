/* Compile like so:
    gcc -o lifeGL -lSDL -lGL -lGLU lifeGL.c
*/
...
static void draw_cell (int x , int y)
{
   ...
   default    : glColor3ub(0x00,0x00,0x50) ;
   }
   
   glBegin(GL_QUADS);
   glVertex2i(x*SIZE, y*SIZE);
   glVertex2i(x*SIZE+SIZE-1, y*SIZE);
   glVertex2i(x*SIZE+SIZE-1, y*SIZE+SIZE-1);
   glVertex2i(x*SIZE, y*SIZE+SIZE-1);
   glEnd();
}
...
void main (int argc, char *argv[])
{
   ...
   screen = SDL_SetVideoMode( WIDTH*SIZE, HEIGHT*SIZE, 0, SDL_HWSURFACE | SDL_DOUBLEBUF | SDL_OPENGL);
   if (screen == NULL) {
     fprintf(stderr, "Video mode setup failure: %s\n", SDL_GetError());
     exit(1);
   }
